...
class PublisherConsumerTests(unittest.TestCase):
    ...
    def test_topic(self):
        ...
        def callback_topic(exchange, routing_key, headers, body):
            print("*** topic callback, exchange: {}, routing_key: {}, body: {}, headers: {}".format(exchange, routing_key, headers, body))
            condition.set()
    ...
    def test_many_topics_same_topic_different_exchanges(self):
        ...
        def callback_topic_one(exchange, routing_key, headers, body):
            print("*** topic ONE callback, exchange: {}, routing_key: {}, body: {}, headers: {}".format(exchange, routing_key, headers, body))
        ...
        # differents exchange, so routing_key can be the same
        the_publisher = publisher.Publisher(utils.create_connection(broker=BROKER_TEST))
    ...
    def test_fanout(self):
        ...
        def callback_worker_one(exchange, routing_key, headers, body):
            print("*** topic at worker ONE callback, exchange: {}, routing_key: {}, body: {}, headers: {}".format(exchange, routing_key, headers, body))
            condition1.set()
        ...
        # same exchange, same routing_key, differents queues. It can be different workers
        worker = consumer.Worker(broker=BROKER_TEST).\
                                add_topic(exchange=EXCHANGE_TEST,
                                          routing_key="amqppy.test.topic",
    ...
    def test_round_robin(self):
        ...
        # same exchange, same routing_key, same queue. It can be different workers
        worker = consumer.Worker(broker=BROKER_TEST).\
                                add_topic(exchange=EXCHANGE_TEST,
                                          routing_key="amqppy.test.topic",
                                          request_func=callback_worker_one,
